 ...
 */
class DIP extends AbstractOrderSearch
{
    /**
     * @param null $filters
    ...
    public function getOrders($filters = null, $pagination = null, $sortOrder = null, $sortField = null): array
    {
        $basic_orders_query = $this->orderQuery($filters, $pagination)
            ->andWhere(['order.order_status' => OrderStatus::DELIVERY_IN_PROGRESS]);
        ...
        $orders_query = $this->orderViewQuery($orders_hashes)
            ...
            ->andWhere(['order_view.order_status' => OrderStatus::DELIVERY_IN_PROGRESS]);
        ...
        return [
            'orders' => $orders,
            'count'  => [
                'count_all' => $this->countOrdersForPagination($filters, $pagination)->andWhere(['order.order_status' => OrderStatus::DELIVERY_IN_PROGRESS])->count()
            ]
        ];
    }
    ...
    protected function selectFields(): array
    {
        return [
            ...
            //'(select COUNT(order_id) from order_view where customer_id = order_view.customer_id and order_id != order_view.order_id) as history',
