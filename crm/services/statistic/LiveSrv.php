...
class LiveSrv
{
    ...
    private function query()
    {
        ...
        if (isset($this->filters['wm_id'])) $query->andWhere(['target_wm.wm_id' => $this->filters['wm_id']]);
    ...
    public function offer()
    {
        ...
        $this->query->andWhere(['!=', 'offer_status', Offer::STATUS_ARCHIVED]);
        $result = $this->getRate($this->days($group_field), $this->actualYesterday($group_field));
        return $result;
    }
    ...
    public function superWm()
    {
        ...
        $this->query->join('LEFT JOIN', 'user WU', 'WU.id = target_wm.wm_id');
        $group_field = 'WU.id';
        $result = $this->getRate($this->days($group_field), $this->actualYesterday($group_field));
        return $result;
    }
    ...
    private function dayQuery($day_number)
    {
...
//                    else {
//                        if (!isset($result[$identifier]['identifier']) && !empty($identifier) && !is_null($identifier)) $result[$identifier]['identifier'] = $identifier;
//                    }

                    $result[$identifier][$day['day']] = intval($row['orders_count']);
                    if (!isset($result[$identifier]['title'])) $result[$identifier]['title'] = $row['title'];
                    if (!isset($result[$identifier]['identifier'])) $result[$identifier]['identifier'] = $row['identifier'];
