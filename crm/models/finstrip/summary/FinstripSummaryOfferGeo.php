...
/**
 * Class FinstripSummaryOfferGeo
 * @package crm\models\finstrip\summary
 */
class FinstripSummaryOfferGeo extends Finstrip
{
    public $geo_id = null;
     ...
     * @return array
     */
    public function searchSummaryOfferGeo($filters)
    {
     ...
     * @return array
     */
    private function compareData($views, $orders, $subs)
    {
        $result = [];
...
//                'IFNULL((SELECT sum(day_offer_geo_sub_cost.usd_sum) FROM day_offer_geo_sub_cost WHERE DATE_FORMAT(day_offer_geo_sub_cost.date,"%m.%Y") = DATE_FORMAT(`order`.created_at,"%m.%Y") AND day_offer_geo_sub_cost.offer_id = ' . $filters['offer_id']['value'] . ' ), 0) AS total_traffic_cost'
            ])
    ...
    private function subsQuery($filters)
    {
        $query = DayOfferGeoSubCost::find()
            ...
            ->where('DATE_FORMAT(day_offer_geo_sub_cost.date,"%m.%Y") = "' . $filters['month']['value'] . '"')
            ->andWhere(['day_offer_geo_sub_cost.offer_id' => $filters['offer_id']['value']]);

        if (isset($filters['advert_id'])){
            $query->addSelect([
                'sum(day_offer_geo_sub_cost.usd_sum) AS total_traffic_cost'
            ]);
            $query->andWhere(['offer_id' => $this->getAdvertOffers($filters['advert_id']['value'])]);
            $query->andWhere(['geo_id' => $this->getAdvertGeo($filters['advert_id']['value'])]);
        } else {
            $query->addSelect([
