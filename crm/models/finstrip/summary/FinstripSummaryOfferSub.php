...
/**
 * Class FinstripSummaryOfferSub
 * @package crm\models\finstrip\summary
 */
class FinstripSummaryOfferSub extends Finstrip
{
    private $knownSubs;
     ...
     * @return array
     */
    public function searchSummaryOfferSub($filters)
    {
    ...
    private function viewsQuery($filters)
    {
        $query = LandingViews::find()
            ...
            ->where('DATE_FORMAT(landing_views.date,"%d.%m.%Y") = "' . $filters['day']['value'] . '"')
            ->andWhere(['landing_views.geo_id' => $filters['geo_id']['value']])
...
//                'sum(CASE WHEN `order`.order_status >= advert_offer_target.advert_offer_target_status THEN `order`.usd_advert_commission ELSE 0 END) AS total_advert_commission'
            ...
            ->andWhere(['`customer`.country_id' => $filters['geo_id']['value']]);
    ...
    private function subsQuery($filters)
    {
        return DayOfferGeoSubCost::find()
            ...
            ->andWhere(['day_offer_geo_sub_cost.geo_id' => $filters['geo_id']['value']])
            ->groupBy(['day_offer_geo_sub_cost.known_sub_id', 'day_offer_geo_sub_cost.sub_id_1'])
            ->orderBy(['day_offer_geo_sub_cost.known_sub_id' => SORT_ASC])
            ->asArray()
            ->all();
    }
}