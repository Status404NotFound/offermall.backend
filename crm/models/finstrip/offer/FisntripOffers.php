 ...
 *
 */
class FisntripOffers extends Finstrip
{
     ...
     * @return array
     */
    public function searchOffers($filters): array
    {
...
WHERE day_offer_geo_sub_cost.offer_id = `order`.offer_id AND day_offer_geo_sub_cost.offer_id IN ('.implode(',', $this->getAdvertOffers($filters['advert_id']['value'])).') AND day_offer_geo_sub_cost.geo_id IN ('.implode(',', $this->getAdvertGeo($filters['advert_id']['value'])).')), 0) AS total_traffic_cost'
            ]);
            ...
            $query->addSelect([
                'IFNULL((SELECT sum(day_offer_geo_sub_cost.usd_sum) 
FROM day_offer_geo_sub_cost 
WHERE day_offer_geo_sub_cost.offer_id = `order`.offer_id), 0) AS total_traffic_cost'
            ]);
        }
