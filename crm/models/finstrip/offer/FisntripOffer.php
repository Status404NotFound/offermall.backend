...
/**
 * Class FisntripOffer
 * @package crm\models\finstrip\offer
 *
 * @property integer $geo_id
 * @property string $geo_name
 *
 */
class FisntripOffer extends Finstrip
{
    public $geo_id = null;
     ...
     * @return array
     */
    public function searchOffer($filters): array
    {
    ...
    /**
     * @param $filters
     * @return array|LandingViews[]|DayOfferGeoSubCost[]|\yii\db\ActiveRecord[]
     */
...
//                'sum(CASE WHEN `order`.order_status >= advert_offer_target.advert_offer_target_status THEN `order`.usd_advert_commission ELSE 0 END) AS total_advert_commission',
    ...
    private function subsQuery($filters)
    {
        $query = DayOfferGeoSubCost::find()
            ...
            ->where(['day_offer_geo_sub_cost.offer_id' => $filters['offer_id']['value']]);
