...
class Turbosms extends Component
{
    ...
    protected function connect()
    {
        ...
        if (is_soap_fault($client)) {
            trigger_error("SOAP Fault: (faultcode: {$client->faultcode}, faultstring: {$client->faultstring})", E_USER_ERROR);
        }

        if (!$this->login || !$this->password) {
//            throw new InvalidConfigException('Enter login and password from Turbosms');
            echo 'Enter login and password from Turbosms';
        }
