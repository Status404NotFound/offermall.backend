...
use common\modules\user\models\forms\RelationsForm;
...
class AdminController extends Controller {
    ...
    /**
     * Event is triggered before updating existing user's relations.
     * Triggered with \common\modules\user\events\UserEvent.
     */
    const EVENT_BEFORE_RELATIONS_UPDATE = 'beforeRelationUpdate';

    /**
     * Event is triggered after updating existing user's relations.
     * Triggered with \common\modules\user\events\UserEvent.
     */
    const EVENT_AFTER_RELATIONS_UPDATE =   'afterRelationUpdate';
    ...
    /**
     * If "user" module is installed, this page displays form
    ...
    public function actionRelations($id)
    {
        ...
        /** @var RelationsForm $relation */
        $relation = \Yii::createObject([
            'class'    => RelationsForm::className(),
            'user' => $user,
        ]);

        $event = $this->getUserEvent($user);

        $this->performAjaxValidation($relation);

        $this->trigger(self::EVENT_BEFORE_RELATIONS_UPDATE, $event);

        if ($relation->load(\Yii::$app->request->post()) && $relation->setRelations()) {
            \Yii::$app->getSession()->setFlash('success', \Yii::t('user', 'Communication between users it is set'));
            $this->trigger(self::EVENT_AFTER_RELATIONS_UPDATE, $event);
            return $this->refresh();
        }

        return $this->render('_relation', [
            'user'    => $user,
            'modelParent' => $relation,
        ]);
    }
