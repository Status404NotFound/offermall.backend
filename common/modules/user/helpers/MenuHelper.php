...
use common\modules\user\models\tables\Menu;
 ...
 * 
 * ```
 * use common\modules\user\components\MenuHelper;
...
class MenuHelper {
    ...
    public static function getAssignedMenu($userId, $root = null, $callback = null, $refresh = false)
    {
        ...
        $key = [__METHOD__, $userId, $authManager->defaultRoles];
        ...
        if ($refresh || $cache === null || ($assigned = $cache->get($key)) === false) {
            ...
            if ($cache !== null) {
                $cache->set($key, $assigned, UserCache::cacheDuration(), new TagDependency([
