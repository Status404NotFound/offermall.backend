...
class Menu extends \yii\widgets\Menu
{
    ...
    public $profileTemplate = '<div class="dropdown profile-element">
                        <span><img alt="image" class="img-circle" src="{avatar}" width="48" height="48"/></span>
    ...
    protected function renderItem($item)
    {
        ...
        if (isset($item['profile'])) {
            ...
            $template = ArrayHelper::getValue($item, 'template', $this->profileTemplate);
            $menu = Html::ul(
                ArrayHelper::getValue($profile, 'items', []),
                [
            ...
            $replace = [
                '{avatar}' => ArrayHelper::getValue($profile, 'avatar', '#'),
                '{name}' => ArrayHelper::getValue($profile, 'name', 'username'),
                '{role}' => ArrayHelper::getValue($profile, 'role', 'role'),
                '{menu}' => ArrayHelper::getValue($profile, 'menu', $menu),
                '{logo}' => ArrayHelper::getValue($profile, 'logo', 'Logo'),
            ];
